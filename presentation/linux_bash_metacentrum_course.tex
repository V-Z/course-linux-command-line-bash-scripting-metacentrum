...
\begin{frame}{The course information}
	\begin{itemize}
		\item The course page: \url{https://trapa.cz/en/course-linux-command-line-2026}
		\begin{itemize}
			\item Česky: \url{https://trapa.cz/cs/kurz-prikazove-radky-linuxu-2026}
		\end{itemize}
...
\begin{frame}[allowframebreaks]{What it is UNIX, Linux and GNU}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Powering everything from little \href{https://en.wikipedia.org/wiki/Internet_of_things}{IoT} toys and devices to 100\% of \href{https://www.top500.org/lists/top500/}{top 500 supercomputers}
		\end{itemize}
...
\begin{frame}[allowframebreaks]{Free and open-source software --- (F)OSS}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Often available for free (e.g. on services like \href{https://github.com/}{GitHub}, \href{https://gitlab.com/}{GitLab}, \href{https://bitbucket.org/}{Bitbucket} or \href{https://sourceforge.net/}{SourceForge})
...
\begin{frame}{Graphical User Interfaces (GUI)}{More like \enquote{Mac-style}, \enquote{Windows-style} or something else? Feature rich or minimalistic?}
	\begin{itemize}
		...
		\item And much \href{https://en.wikipedia.org/wiki/Desktop_environment\#Examples_of_desktop_environments}{more}\ldots
...
\begin{frame}{Short overview of hard disk layout}
	\begin{itemize}
		...
		\item GPT is a \enquote{table} describing partitioning of physical disk
		\begin{itemize}
			\item Replaces older MBR
			\item Requires UEFI --- basic computer firmware responsible of booting of the computer (replacement of old BIOS)
		\end{itemize}
...
\begin{frame}[allowframebreaks]{Directory structure in Linux}
	\begin{itemize}
		...
		\item \alert{\texttt{/media}} --- attached disks (USB flash,~\ldots) usually appear there (might be in \texttt{/var/run/media}, \texttt{/var/media} or \texttt{/run/media}) --- subdirectories are automatically created when device is plugged and disappears when unplugged
		...
		\item \texttt{/proc} --- dynamic information about system processes (often also pluggable media appear here)
		...
		\item \texttt{/tmp} --- temporary files (content use to be discarded during reboot) --- users have private dynamically created spaces there, used automatically by applications according to need
...
\begin{frame}[fragile]{Links}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item When we delete link, nothing happens; when target is deleted, non-working link remains
		\end{itemize}
	\end{itemize}
	\vfill
	\begin{bashcode}
...
\begin{frame}{Owner and group}
	\begin{itemize}
		...
		\item Ownership (and permissions, slide~\ref{permissions}) are important especially on servers with plenty of users (e.g. on MetaCentrum) --- share files with only dedicated users, protect private files
...
\begin{frame}[allowframebreaks]{Extending permissions --- ACL (Access control list)}
	...
	\begin{itemize}
		...
		\item MetaCentrum has storages and clusters connected via NFSv4 protocol (see also slide~\ref{netfs}) --- commands \texttt{getfacl} and \texttt{setfacl} do not work there, use \texttt{nfs4\_getfacl}, \texttt{nfs4\_setfacl} and \texttt{nfs4\_editfacl} instead (usage is very similar), see also \href{https://docs.metacentrum.cz/en/docs/data/data-sharing\#nfsv4-wrapper}{MetaCentrum documentation} and its wrapper script
...
\begin{frame}{Set default permissions for new files}
	\begin{itemize}
		...
		\item Syntax is similar to \texttt{chmod}, but reverse (e.g. \texttt{027} keeps all rights for owner, for group removes writing and nothing left for the others)
		\begin{itemize}
...
\begin{frame}[fragile]{Login to remote server}{SSH --- secure shell --- encrypted connection}
    ...
    ssh courseuXY@vyuka.natur.cuni.cz
		...
		\begin{itemize}
			...
			\item From Windows use \href{https://www.chiark.greenend.org.uk/~sgtatham/putty/}{Putty} (see right figure)
		\end{itemize}
		\begin{center}
...
\begin{frame}[fragile]{Brace expansion and quotes}
	...
	\begin{itemize}
		...
		\item Text between back ticks (\texttt{\textasciigrave\ldots\textasciigrave}) or within \texttt{\$(\ldots)} will be evaluated and then used as command or argument (see further for examples)
	\begin{itemize}
...
\begin{frame}[fragile]{Redirects of input/output}
	...
	\end{itemize}
	\begin{bashcode}
    echo "Žluťoučký kůň úpěl ďábelské šó" | iconv -f utf-8 -t cp1250 /dev/stdin
	\end{bashcode}
	\begin{itemize}
...
\begin{frame}[allowframebreaks]{Network protocols}
	\begin{itemize}
		...
		\item Messaging protocols like XMPP (Jabber), IRC, Discord, MS Teams, Slack, Matrix,~\ldots
...
\begin{frame}[fragile]{Connect to SSH with key}{No need to remember password for every server\ldots}
	\begin{bashcode}
    # Create the key (several options) --- for most of cases keep defaults
...
\begin{frame}{Parallelization with GNU Parallel}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \texttt{:::} --- use arguments from command line instead of stdin (\texttt{:::} is placed after the command and before the argument, see further examples)
...
\begin{frame}[fragile]{Work with columns}
    ...
    cut -c 1 /etc/group # Get first character from each line
    ...
    cut -d ':' -f 1 /etc/group # Select 1st field, fields are separated by ":"
    ...
    # Swapping columns is not very comfortable (see later AWK)...
...
\begin{frame}[fragile]{Examples of usage of head, grep and cut with our data I}
    ...
    ls *.sh | wc -l # How many BASH script are in current directory
	\end{bashcode}
\end{frame}
...
\begin{frame}[fragile]{Get a~column with awk}
	\begin{itemize}
		...
		\item Has complicated syntax, it is hard to read, it is not similar to other tools --- Perl can do more and is more common\ldots
...
\begin{frame}[fragile]{Sorting I}
    ...
    LC_ALL=C sort ... # Force usage of English locale
...
\begin{frame}[fragile]{Sed examples I}
	...
	\end{itemize}
	\begin{bashcode}
    sed 'operator/FindToReplace/Replacement/modificator' textfile > newtextfile
...
\begin{frame}[fragile]{Multiple switches in classical UNIX form (no positional) V}
	\begin{itemize}
		\item Script \texttt{interactive5.sh} contains complete example --- take inspiration there
...
\begin{frame}[fragile]{Simple providing of input file II}
	...
	\end{bashcode}
	\begin{itemize}
		\item Similar (even simpler) example is typical MetaCentrum script (slide~\ref{metacentrumscript})
\end{itemize}
\end{frame}
...
\begin{frame}[allowframebreaks]{Evaluation of conditions}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \textbf{Regular expressions:} \texttt{if [[ "\$\textbraceleft STRINGVAR\textbraceright " \alert{=$\sim$} .[sS]tring.* ]] \ldots} --- \texttt{.} represents any single character (\texttt{?} would be zero or one occurrence of preceding expression), \texttt{[]} any character inside and \texttt{.*} zero or more occurrences of any single characters
...
\begin{frame}[allowframebreaks]{Package management}{Installation of software in Linux}
	\begin{itemize}
...
\begin{frame}[allowframebreaks]{Package management in command line in openSUSE and SLE (basic commands)}
	\begin{itemize}
		...
		\item \texttt{zypper dup} or \texttt{opensuse-migration-tool} -- upgrade to newer release of whole distribution (see \href{https://en.opensuse.org/SDB:System_upgrade}{documentation})
		...
		\item \texttt{zypper ar \textit{repository}} --- add \textit{repository} (URL of remote \texttt{*.repo} file etc.)
		\item \texttt{zypper rr \textit{repository}} --- remove \textit{repository} (name according to \texttt{zypper lr})
		\item \texttt{zypper mr \ldots{ }\textit{repository}} --- modify \textit{repository} (see \texttt{man zypper} first, or use \texttt{yast sw\_single})
...
\begin{frame}[allowframebreaks]{Package management in command line in RedHat, Fedora, CENTOS and derivatives (basic commands)}
	\begin{itemize}
		...
		\item \texttt{man dnf} (or \texttt{man yum} --- older tool to do the same tasks), \texttt{man rpm} --- usage help
	\end{itemize}
\end{frame}
...
\begin{frame}{Manuals for package management}
	\begin{itemize}
		...
		\item Red Hat, Fedora, CENTOS, and others: \url{https://docs.fedoraproject.org/en-US/quick-docs/dnf/} and older \url{http://yum.baseurl.org/}
...
\begin{frame}[fragile]{Basics of compilation}
	\begin{itemize}
		...
		\item \textbf{If you don't have to do it, don't do it.}\textbf{} Solving problems can be complicated --- contact someone skilled or author of the application, consult AI\ldots
	\end{itemize}
	\begin{bashcode}
...
\begin{frame}[fragile]{Compilation of RAxML}
    ...
    cd standard-RAxML/ # Go to the downloaded source code
    ...
    ls # List files. What changed? Any new items?
    ./raxmlHPC -h # Launch it - see RAxML help
	\end{bashcode}
\end{frame}

\begin{frame}[fragile]{Compilation of SAMtools} % TODO Use Git instead os wget. New version?
	\begin{itemize}
		\item Ensure packages \texttt{zlib} and \texttt{zlib-dev(el)} are installed --- required for running and compilation, see \href{https://github.com/samtools/samtools/blob/master/INSTALL}{INSTALL} and \href{https://github.com/samtools/samtools/blob/master/README}{README}, and \url{https://www.htslib.org/download/}
	\end{itemize}
	\vfill
	\begin{bashcode}
    ...
    less README # See documentation (also check INSTALL)
...
\begin{frame}[fragile]{Launching Java applications}
	\begin{itemize}
		...
		\item Let's download e.g. \href{https://tree.bio.ed.ac.uk/software/figtree/}{FigTree} from \url{https://github.com/rambaut/figtree/releases}
	\end{itemize}
	\vfill
	\begin{bashcode}
    # Go to directory where you downloaded the latest version
    cd directory/with/downloaded/figtree
    # Decompress downloaded archive
    tar zxvf FigTree_v1.4.5_pre.tgz
    # Go to created directory
    cd FigTree_v1.4.5_pre/
...
\begin{frame}[allowframebreaks]{Windows applications on Linux}
	\begin{itemize}
		...
		\end{itemize}
		\item Windows has since v. 10 \href{https://learn.microsoft.com/windows/wsl/install}{possibility to run Linux applications}, other option is \href{https://www.cygwin.com/}{Cygwin} (application must be specially compiled to work on Cygwin)
...
\begin{frame}[allowframebreaks]{Where to get scientific applications for Linux}
	\begin{itemize}
		...
		\end{itemize}
		\item Scientific programming languages like R, Python, Perl, Julia, Matlab (Octave) etc. have their own system to install language-specific packages
		\begin{itemize}
...
\begin{frame}{How to game on Linux}
	\begin{itemize}
		...
		\item Although not all games available for Windows are available for Linux, there are more games than you have time to play\ldots~;-)
	\end{itemize}
\end{frame}
...
\begin{frame}[allowframebreaks]{CESNET and MetaCentrum}
	\label{CESNET}
	\begin{itemize}
		\item \href{https://www.cesnet.cz/en}{CESNET} (\href{https://www.cesnet.cz/}{česky}) is organization of Czech universities, Academy of Science and other organizations taking care about Czech backbone Internet (among other tasks), one of world leading institutions of this type
		\item CESNET also provides various \href{https://www.cesnet.cz/en/services}{services} (\href{https://www.cesnet.cz/sluzby/}{česky})
		\begin{itemize}
		...
		\begin{itemize}
			...
			\item To use data storage fill registration form \url{https://einfra.cesnet.cz/allfed/registrar/?vo=VO_s3&group=s3_cl4}
		...
		\item Information about MetaCloud \url{https://docs.platforms.cloud.e-infra.cz/}
		\item Also available is Galaxy \url{https://usegalaxy.cz/} (same login as to MetaCentrum) --- web based bioinformatics framework (more information at \href{https://docs.metacentrum.cz/en/docs/related/galaxy}{documentation})
		...
		\item List of available applications \url{https://docs.metacentrum.cz/en/docs/software/alphabet}
		\item MetaCentrum has 9~\href{https://docs.metacentrum.cz/en/docs/computing/infrastructure/frontends}{frontends} where users log-in, and thousands of computers doing the calculations --- they are not accessed directly to run task, tasks are submitted via frontends
...
\begin{frame}[allowframebreaks]{Distributed topology}
	\begin{itemize}
		...
		\item There are special \href{https://docs.du.cesnet.cz/en/docs/introduction/description}{archive storages} in multiple locations
	\end{itemize}
	\begin{center}
	...
	\begin{itemize}
		...
		\item There are 11 \href{https://docs.metacentrum.cz/en/docs/computing/infrastructure/frontends}{frontends} where user logs via SSH and prepare computing task for submission etc. (see further)
		...
		\item Some special services (like \href{https://docs.metacentrum.cz/en/docs/graphical/ondemand}{OnDemand}) start on particular storage
...
\begin{frame}[fragile]{MetaCentrum usage}
	\begin{enumerate}
		\item \href{https://docs.metacentrum.cz/en/docs/data/large-data}{Transfer} data into one of \href{https://docs.metacentrum.cz/en/docs/computing/infrastructure/storages}{storages} by e.g. \texttt{scp} from command line, \href{https://winscp.net/}{WinSCP} from Windows or \href{https://filezilla-project.org/}{FileZilla} from any OS
		\item Connect via SSH to selected \href{https://docs.metacentrum.cz/en/docs/computing/infrastructure/frontends}{frontend}. Same credentials are used for all frontends, for SSH login as well as file transmissions
...
\begin{frame}{File transfers to MetaCentrum}
	\begin{multicols}{2}
		\begin{itemize}
			...
			\item Protocol is SSH/SSH2/SFTP/SCP, port 22, server is selected \href{https://docs.metacentrum.cz/en/docs/computing/infrastructure/frontend-storage}{storage} address --- if possible, select any and keep using it
...
\begin{frame}[fragile]{Basic skeleton of script running tasks II}
    ...
    # See https://docs.metacentrum.cz/en/docs/software/alphabet
...
\begin{frame}[fragile]{Launching of tasks}
	\begin{itemize}
		\item See \url{https://docs.metacentrum.cz/en/docs/computing/run-basic-job}
...
\begin{frame}[fragile]{Key MetaCentrum commands}
	\begin{itemize}
		...
		\item Command \texttt{module} loads/unloads selected \href{https://docs.metacentrum.cz/en/docs/software/alphabet}{application}
		...
		\begin{itemize}
			\item It has plenty of options how to specify requirements (see next slide and \href{https://docs.metacentrum.cz/en/docs/computing/resources/resources}{help})
		\end{itemize}
...
\begin{frame}[allowframebreaks]{Scheduling details}
	\begin{itemize}
		...
		\end{itemize}
		\item If requesting e-mails (e.g. \texttt{-m abe} to get mail about \textit{abort}, \textit{beginning} and \textit{exit} of the task) and submitting plenty of tasks by some script, it can result in plenty of mails\ldots
		\item Every user has certain \href{https://metavo.metacentrum.cz/pbsmon2/users/}{priority} increased by \href{https://docs.metacentrum.cz/en/docs/access/terms\#acknowledgements-and-publications}{acknowledgments} (\textbf{it's mandatory to acknowledge MetaCentrum when using it}) in publications to MetaCentrum, and lowered by intensive usage of the service (the usage is calculated from past month)
		...
		\item For more options read \url{https://docs.metacentrum.cz/en/docs/computing/resources/resources}
		\begin{itemize}
...
\begin{frame}{Common problems with launching the tasks}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Limiting memory is problematic, e.g. Java allows \texttt{java -Xmx8g -jar app.jar}, some applications also --- check documentation
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Running R tasks on MetaCentrum}
	\begin{itemize}
		\item There are only some R packages installed, to use more create own package library and use it in scripts (see e.g. \texttt{.libPaths()} within \texttt{R})
...
\begin{frame}{MetaCentrum task}
	\begin{enumerate}
		...
		\item Wait for the task to be done and explore output (e.g. open the \texttt{treefile} in \href{https://tree.bio.ed.ac.uk/software/figtree/}{FigTree}).
...
\begin{frame}[fragile]{Graphical interactive task}
	\begin{itemize}
		\item See \url{https://docs.metacentrum.cz/en/docs/software/graphical-access}
...
\begin{frame}{OnDemand}{Applications in web browser}
	\begin{itemize}
		...
		\item See \url{https://docs.metacentrum.cz/en/docs/graphical/ondemand} and \url{https://ondemand.metacentrum.cz/}
		\item Applications start in selected storage --- ensure to have everything needed in correct place
	\end{itemize}
	\begin{center}
...
\begin{frame}{MetaCentrum cloud}
	\begin{itemize}
		\item MetaCentrum \href{https://docs.platforms.cloud.e-infra.cz/}{cloud} allows to run complete Linux distribution (Debian, CentOS,~\ldots), where you can install any software, like on any other computer
		\item Useful e.g. for various testing or applications with very special requirements
		\item Go to \url{https://cloud.metacentrum.cz/} and see \href{https://docs.platforms.cloud.e-infra.cz/}{documentation}
...
\begin{frame}{Git}
	\begin{multicols}{2}
		...
		\vfill
		\begin{center}
			\includegraphics[height=3.25cm]{git_fire.png}
		\end{center}
		\columnbreak
		\begin{center}
			\includegraphics[height=5.25cm]{git_xkcd.png}
		\end{center}
		\begin{flushright}
...
\begin{frame}[fragile]{Working with Git --- tags, logs and more}
    ...
    git reset --hard origin/main # If local repository is broken...
	\end{bashcode}
\end{frame}

\begin{frame}[fragile]{Working with Git --- summary, settings and more}
	\begin{itemize}
		\item Basically repeat in \texttt{git} commands \texttt{add}, \texttt{commit} and \texttt{push} to get your work to the repository, and \texttt{fetch} and \texttt{pull} to download news from the central repository
		\item \texttt{diff} to see local changes, \texttt{status} to see state
		\item \texttt{log} and \texttt{branch} show history and branches
		\item \texttt{branch}, \texttt{checkout} and \texttt{merge} for branching code and merging changes back to main (master) branch
	\end{itemize}
	\begin{bashcode}
...
\begin{frame}[fragile]{Git tasks}
	\begin{bashcode}
    git clone https://project_243_bot_9fce660bb409b4d418f7052c1f375914:
      glpat-vkWesRqxGBOUEw9MMxy3pm86MQp1OjM0CA.01.0y1xsw5ib@git.sorbus.
      ibot.cas.cz/restio/dadaism.git # It is one long line (no space after : .)
	\end{bashcode}
	\begin{enumerate}
		...
		\item Push your changes back to the repository. Fetch changes done by others.
...
\begin{frame}{ChatGPT and others}
	\begin{itemize}
		...
		\item \alert{Never ever paste private data or data under copyright}
	\end{itemize}
\end{frame}
...
\begin{frame}{Fedora}
	\begin{itemize}
		...
		\item Communication and help overview \url{https://docs.fedoraproject.org/en-US/project/communications/}
...
\begin{frame}{GNOME, KDE and XFCE}
	\begin{itemize}
		...
		\begin{itemize}
			\item Help for users \url{https://help.gnome.org/}
