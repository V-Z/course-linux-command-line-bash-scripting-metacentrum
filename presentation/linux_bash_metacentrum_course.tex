...
\begin{frame}{Redirects and pipes}
	\label{pipe}
	\begin{itemize}
		\item \texttt{/dev/null} --- \enquote{black hole}
		\begin{itemize}
			\item Can discard anything
			\item Discard only errors (note \enquote{2}): \texttt{command 2\textgreater~/dev/null}
		\end{itemize}
...
\begin{frame}{Emacs basics}
	\begin{itemize}
		...
		\item Keyboard commands are noted as e.g. \texttt{C-x}, where \enquote{\texttt{C}} stands for (mostly) \texttt{Ctrl} key, \texttt{M-x}, where \enquote{\texttt{M}} stands mostly for \texttt{Alt} key, sometimes for \texttt{Win} key
	\end{itemize}
	\begin{multicols}{2}
		\begin{itemize}
