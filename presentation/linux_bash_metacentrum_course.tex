...
% TODO More about usage of VB appliance
...
% TODO Other common UNIX systems (BSD, Solaris, Mac OS X, ...)
...
\begin{frame}[allowframebreaks]{Free and open-source software --- (F)OSS}
  \begin{itemize}
    ...
    \item BSD license --- permissive --- allow derived code to became closed-source (commonly used by e.g. Apple Mac OS~X, Safari browser,~\ldots)
...
\begin{frame}{Ubuntu with Unity} % TODO Update prin screen, add more
  \begin{center}
    \includegraphics[height=7cm]{ubuntu.png}
  \end{center}
\end{frame}

\begin{frame}{openSUSE with KDE --- Kubuntu is same, but blue\ldots} % TODO Update prin screen, add more
  \begin{center}
    \includegraphics[height=7cm]{opensuse.png}
  \end{center}
\end{frame}

\begin{frame}{Fedora with GNOME --- GNOME is always almost same} % TODO Update prin screen, add more
  \begin{center}
    \includegraphics[height=7cm]{fedora.png}
  \end{center}
\end{frame}

\begin{frame}{Linux Mint with Cinnamon} % TODO Update prin screen, add more
  \begin{center}
    \includegraphics[height=7cm]{mint.png}
  \end{center}
\end{frame}

\begin{frame}{Debian with XFCE --- Xubuntu has more ``modern'' design} % TODO Update prin screen, add more
  \begin{center}
...
\begin{frame}[allowframebreaks]{How to try it?} % TODO Add Windows 10 Linux subsystem and Cygwin
  \begin{itemize}
...
\begin{frame}{File systems} % TODO add ZFS
  \begin{center}
    \begin{footnotesize}
...
\begin{frame}[allowframebreaks]{Creation and control of FS}
  \begin{itemize}
    ...
    \item Users must be sure what is doing, otherwise system can be damaged
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Root vs. ``normal'' user} % TODO Add XKCD cartoon
  \begin{itemize}
...
% TODO Everything is file

\begin{frame}[fragile]{File names} % TODO limits of names, limits of FAT32
  \begin{itemize}
...
\begin{frame}{Configuration in /etc (examples)} % TODO Add example of configuration file
  \begin{itemize}
...
\begin{frame}[fragile]{Change owner and/or group} % TODO Add more commands
  \begin{bashcode}
...
\begin{frame}[fragile]{Converting the text}{Prevent bad display and weird errors when launching scripts}
  ...
  \end{bashcode}
  \begin{itemize}
    \item Mac OS~X uses same encoding and EOL as Linux (and rest of UNIX world), so there are no problems with compatibility
...
\subsection[SSH]{SSH --- secure shell and screen} % TODO Add task to login to remote server
...
\begin{frame}{The shell} % TODO Add print screen
  \begin{itemize}
...
\begin{frame}[fragile]{Directories} % TODO Add some tasks
  \begin{bashcode}
...
\begin{frame}[fragile]{Find examples} % TODO Examples with toy files
  \begin{bashcode}
...
\begin{frame}[allowframebreaks]{Network protocols} % TODO links to respective slides
  \begin{itemize}
...
\begin{frame}[fragile]{Connecting file systems on remote servers} % TODO Add task to use sshfs
  \begin{itemize}
    \item Target mount point must exist before mounting
...
\begin{frame}[fragile]{Transferring files from/to remote server} % TODO Add task to get data from training server
  \label{transfers}
  \begin{itemize}
...
\begin{frame}[fragile]{Synchronization with rsync} % TODO Add task to get data from training server
  \begin{itemize}
...
\subsection{Parallelisation} % TODO Examples with toy files
...
\subsection{Reading} % TODO Examples with toy files
...
\subsection{Extractions} % TODO Examples with toy files
...
\subsection{AWK} % TODO Examples with toy files, with FASTQ
...
\subsection{Manipulations} % TODO Examples with toy files
...
\begin{frame}{The editors and their usage} % TODO More about Emacs and Vim
  \begin{itemize}
...
\subsection{Regular expressions} % TODO Examples with toy files, more examples
...
\begin{frame}[allowframebreaks]{Regular expressions}
  ...
  \begin{itemize}
    ...
    \item Mac OS~X has by default very outdated version of \texttt{sed} and another tools --- it does not have all advanced features --- users need to install e.g. \texttt{gnu-sed} formulae from \href{https://brew.sh/}{Homebrew}
  \end{itemize}
\end{frame}

\section{Scripting} % TODO Update according to modified examples
...
\subsection{Branching the code} % TODO Add more examples
...
\subsection{Loops} % TODO Examples with toy files
...
\begin{frame}{Package management in command line in openSUSE and Debian/Ubuntu (basic commands)} % TODO Add dnf/yum
...
% TODO Add work with repositories
...
\subsection{Windows applications} % TODO Add some examples
...
\subsection{Usage} % TODO Update to PBS Pro
...
\begin{frame}{File transfers to MetaCentrum} % TODO Update servers
  \begin{itemize}
...
\begin{frame}[fragile]{Basic skeleton of script running tasks I} % TODO Update according to new file version
  \begin{bashcode}
    #!/bin/bash
...
\begin{frame}[fragile]{Launching of tasks} % TODO Update to PBS Pro
  \begin{itemize}
...
\begin{frame}[allowframebreaks]{Scheduling details} % TODO Update to PBS Pro
  \begin{itemize}
...
% TODO More examples about software on MetaCentrum, scripts (on MetaCentrum)
...
% TODO Security, antiviruses

\section{Git}

% TODO More general info about Git, separate GitHub to extra slide
...
\subsection{Resources} % TODO Possibly add more resources
...
\begin{frame}[allowframebreaks]{How to ask for help} % TODO Update, inspire in R
  \label{howtoask}
  \begin{itemize}
